...
public class CraftingTableCommand implements CommandExecutor {
    ...
    public CraftingTableCommand(ServerEssentials plugin) {
        this.plugin = plugin;
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!(sender instanceof Player player)) {
            // Send message to the sender (console or non-player)
            sender.sendMessage(miniMessage.deserialize("<red>Only players can use this command.</red>"));
            return true;
        }

        // Open the real Crafting Table GUI
        player.openWorkbench(null, true);

        // Send configurable MiniMessage message
        plugin.adventure().player(player).sendMessage(getMessage("opened-crafting-table"));
        return true;
    }
