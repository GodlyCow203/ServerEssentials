...
public class MessagesManager {
    ...
    public void load(String fileName) {
        ...
        loadedFiles.put(fileName, newConfig); // overwrite previous
    }
    ...
    public void reload(String fileName) {
        File file = new File(plugin.getDataFolder(), "messages/" + fileName);
        if (!file.exists()) {
            plugin.getLogger().warning("[MessagesManager] File messages/" + fileName + " not found, creating a new one.");
            plugin.saveResource("messages/" + fileName, false);
        }

        try {
            FileConfiguration newConfig = YamlConfiguration.loadConfiguration(file);
            loadedFiles.put(fileName, newConfig);
            plugin.getLogger().info("[MessagesManager] Reloaded messages/" + fileName + " successfully.");
        } catch (Exception e) {
            plugin.getLogger().severe("[MessagesManager] Failed to reload messages/" + fileName + ": " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    public void reloadAll() {
        Map<String, FileConfiguration> reloaded = new HashMap<>();

        for (String fileName : loadedFiles.keySet()) {
            File file = new File(plugin.getDataFolder(), "messages/" + fileName);
            if (!file.exists()) {
                plugin.getLogger().warning("[MessagesManager] File messages/" + fileName + " missing, skipping reload.");
                continue;
            }

            FileConfiguration newConfig = YamlConfiguration.loadConfiguration(file);
            reloaded.put(fileName, newConfig);
        }

        loadedFiles.clear();
        loadedFiles.putAll(reloaded);
