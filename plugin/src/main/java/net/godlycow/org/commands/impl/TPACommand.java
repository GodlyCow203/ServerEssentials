...
public class TPACommand implements CommandExecutor {
        ...
        if (!player.hasPermission("essc.command.tpa.bypass.cooldown")) {
            storage.getCooldown(player.getUniqueId()).thenAccept(cooldownTime -> {
                ...
                if (cooldownTime > 0 && (now - cooldownTime) < cd * 1000L) {
                    ...
                    Component msg = langManager.getMessageFor(player, "commands.tpa.cooldown",
    ...
    private void handleTpa(Player sender, String targetName, boolean here) {
        ...
        if (target == null || !target.isOnline()) {
            Component msg = langManager.getMessageFor(sender, "commands.tpa.no-player",
        ...
        if (sender.equals(target)) {
            Component msg = langManager.getMessageFor(sender, "commands.tpa.cannot-tpa-self",
        ...
        storage.saveRequest(request).thenRun(() -> {
            Component senderMsg = langManager.getMessageFor(sender, here ? "commands.tpa.request-sent-here" : "commands.tpa.request-sent",
                    "<green>TPA request sent to {player}.",
                    LanguageManager.ComponentPlaceholder.of("{player}", target.getName())
            );
            sender.sendMessage(senderMsg);

            Component targetMsg = langManager.getMessageFor(target, here ? "commands.tpa.request-received-here" : "commands.tpa.request-received",
    ...
    private void handleAcceptDeny(Player player, String senderName, boolean accept) {
        storage.getActiveRequests(player.getUniqueId()).thenAccept(requests -> {
            if (requests.isEmpty()) {
                Component msg = langManager.getMessageFor(player, "commands.tpa.no-request",
            ...
            if (request == null && senderName != null) {
                Component msg = langManager.getMessageFor(player, "commands.tpa.no-request-from-player",
    ...
    private void handleCancel(Player sender, String targetName) {
        if (targetName == null) {
            storage.getActiveRequestsForSender(sender.getUniqueId()).thenAccept(requests -> {
                if (requests.isEmpty()) {
                    Component msg = langManager.getMessageFor(sender, "commands.tpa.no-requests-sent",
                ...
                requests.forEach(req ->
                        storage.removeRequest(req.senderId, req.targetId).thenRun(() -> {
                            ...
                            if (target != null) {
                                Component msg = langManager.getMessageFor(target, "commands.tpa.request-cancelled-target",
                ...
                Component msg = langManager.getMessageFor(sender, "commands.tpa.request-cancelled-all",
            ...
            storage.removeRequest(sender.getUniqueId(), target.getUniqueId()).thenRun(() -> {
                Component senderMsg = langManager.getMessageFor(sender, "commands.tpa.request-cancelled",
                        "<green>Cancelled TPA request to {player}.",
                        LanguageManager.ComponentPlaceholder.of("{player}", target.getName()));
                sender.sendMessage(senderMsg);

                Component targetMsg = langManager.getMessageFor(target, "commands.tpa.request-cancelled-target",
                        "<yellow>{player} cancelled their TPA request.",
                        LanguageManager.ComponentPlaceholder.of("{player}", sender.getName()));
                target.sendMessage(targetMsg);
            });
    ...
    private void handleTpall(Player sender) {
        ...
        for (Player target : Bukkit.getOnlinePlayers()) {
            ...
            Component msg = langManager.getMessageFor(target, "commands.tpa.request-received-all",
        ...
        Component senderMsg = langManager.getMessageFor(sender, "commands.tpa.request-sent-all",
    ...
    private void handleToggle(Player player) {
        ...
        storage.setToggle(player.getUniqueId(), newState).thenRun(() -> {
            if (newState) {
                ...
                Component msg = langManager.getMessageFor(player, "commands.tpa.toggle-on",
                        "<green>TPA requests are now <bold>DISABLED</bold>.");
                player.sendMessage(msg);
            } else {
                activeToggles.remove(player.getUniqueId());
                Component msg = langManager.getMessageFor(player, "commands.tpa.toggle-off",
            ...
            storage.getActiveRequests(player.getUniqueId()).thenAccept(incoming -> {
                storage.getActiveRequestsForSender(player.getUniqueId()).thenAccept(outgoing -> {
                    if (incoming.isEmpty() && outgoing.isEmpty()) {
                        ...
                        if (!incoming.isEmpty()) {
                            player.sendMessage(langManager.getMessageFor(player, "commands.tpa.info-incoming-header",
                                    "<gold>Incoming requests:").toString());
                            incoming.forEach(req -> {
                                String senderName = Bukkit.getOfflinePlayer(req.senderId).getName();
                                player.sendMessage(langManager.getMessageFor(player, "commands.tpa.info-request",
                                        "<yellow>- {player} ({type})",
                                        LanguageManager.ComponentPlaceholder.of("{player}", senderName),
                        ...
                        if (!outgoing.isEmpty()) {
                            player.sendMessage(langManager.getMessageFor(player, "commands.tpa.info-outgoing-header",
    ...
    private boolean validateRequest(Player sender, Player target) {
        if (activeToggles.contains(target.getUniqueId())) {
            Component msg = langManager.getMessageFor(sender, "commands.tpa.target-toggled",
        ...
        if (!config.crossWorld && !sender.getWorld().equals(target.getWorld())) {
            Component msg = langManager.getMessageFor(sender, "commands.tpa.crossworld-disabled",
        ...
        if (config.blockedWorlds.contains(target.getWorld().getName())) {
            Component msg = langManager.getMessageFor(sender, "commands.tpa.blocked-world",
    ...
    private void startTimeout(TPARequest request) {
        ...
        Bukkit.getScheduler().runTaskLater(plugin, () -> {
            storage.removeRequest(request.senderId, request.targetId).thenRun(() -> {
                ...
                if (sender != null) {
                    Component msg = langManager.getMessageFor(sender, "commands.tpa.request-expired",
                ...
                if (target != null) {
                    Component msg = langManager.getMessageFor(target, "commands.tpa.request-expired-target",
    ...
    private void acceptRequest(TPARequest request) {
        ...
        Component senderMsg = langManager.getMessageFor(sender, "commands.tpa.request-accepted",
        ...
        Component targetMsg = langManager.getMessageFor(target, "commands.tpa.request-accepted-target",
        ...
        if (warmupTicks > 0) {
            sender.sendMessage(langManager.getMessageFor(sender, "commands.tpa.warmup-start",
    ...
    private void denyRequest(TPARequest request) {
        ...
        if (sender != null) {
            Component msg = langManager.getMessageFor(sender, "commands.tpa.request-denied",
        ...
        if (target != null) {
            Component msg = langManager.getMessageFor(target, "commands.tpa.request-denied-target",
