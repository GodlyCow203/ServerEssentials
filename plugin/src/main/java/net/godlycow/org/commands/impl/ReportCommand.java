...
public class ReportCommand implements CommandExecutor, TabCompleter {
    ...
    private boolean handleReport(CommandSender sender, String[] args) {
        ...
        if (!player.hasPermission("essc.command.report")) {
            Component msg = langManager.getMessageFor(player, "commands.no-permission",
                    ...
                    LanguageManager.ComponentPlaceholder.of("{permission}", "essc.command.report"));
            player.sendMessage(msg);
            return true;
        }

        if (args.length < 2) {
            Component msg = langManager.getMessageFor(player, "commandsreports.command.usage",
        ...
        if (target == null || (!target.hasPlayedBefore() && !target.isOnline())) {
            Component msg = langManager.getMessageFor(player, "commands.reports.command.invalid-player",
        ...
        storage.getCooldown(player.getUniqueId()).thenAccept(lastUsed -> {
            ...
            if (now - lastUsed < cdMillis) {
                ...
                Component msg = langManager.getMessageFor(player, "commands.reports.cooldown",
            ...
            if (reason.length() > config.maxReasonLength) {
                Component msg = langManager.getMessageFor(player, "commands.reports.reason-too-long",
            ...
            storage.addReport(report).thenRun(() -> {
                ...
                Component msg = langManager.getMessageFor(player, "commands.reports.command.submitted",
                ...
                player.sendMessage(langManager.getMessageFor(player, "commands.reports.error",
    ...
    private void notifyStaff(Report report, String targetName) {
        Component notifyMsg = langManager.getMessageFor(null, "commands.reports.command.notify",
        ...
        for (Player admin : Bukkit.getOnlinePlayers()) {
            if (admin.hasPermission("essc.command.report.*")) {
    ...
    private boolean handleClear(CommandSender sender, String[] args) {
        ...
        if (args.length < 1) {
            Player player = getPlayerOrNull(sender);
            Component msg = langManager.getMessageFor(player,
                    "commands.reports.commandclear.usage",
        ...
        storage.getReport(id).thenAccept(optReport -> {
            if (optReport.isEmpty()) {
                Player player = getPlayerOrNull(sender);
                Component msg = langManager.getMessageFor(player,
                        "commands.reports.commandclear.invalid",
            ...
            storage.clearReport(id).thenRun(() -> {
                Player player = getPlayerOrNull(sender);
                Component msg = langManager.getMessageFor(player,
                        "commands.reports.commandclear.cleared",
                ...
                Component msg = player != null ?
                        langManager.getMessageFor(player, "commands.reports.error", "<red>Error clearing report. Please try again.") :
                        langManager.getMessageFor(null, "commands.reports.error", "<red>Error clearing report. Please try again.");
    ...
    public void handleJoin(Player player) {
        if (!player.hasPermission("essc.command.report.*")) return;

        storage.getPendingReports().thenAccept(reports -> {
            if (reports.isEmpty()) return;

            Component header = langManager.getMessageFor(player, "commands.reports.reportslist.header",
            ...
            for (Report report : reports) {
                ...
                Component entry = langManager.getMessageFor(player, "commands.reports.reportslist.entry",
