...
public class WarpDeleteCommand extends CommandModule implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length != 1) {
            player.sendMessage(langManager.getMessageFor(player, "delwarp.usage",
        ...
        if (!player.hasPermission("serveressentials.command.delwarp")) {
            player.sendMessage(langManager.getMessageFor(player, "commands.no-permission",
                    ...
                    LanguageManager.ComponentPlaceholder.of("{permission}", "serveressentials.command.delwarp")));
            return true;
        }
        ...
        warpManager.exists(warpName).thenCompose(exists -> {
            if (!exists) {
                player.sendMessage(langManager.getMessageFor(player, "delwarp.not-found",
            ...
            return warpManager.getCreator(warpName).thenCompose(optCreator -> {
                ...
                if (creator == null || (!creator.equals(player.getUniqueId()) &&
                        !player.hasPermission("serveressentials.command.delwarp.others"))) {
                    player.sendMessage(langManager.getMessageFor(player, "delwarp.no-permission",
                            "<red>You don't have permission to delete this warp!"));
                    return CompletableFuture.completedFuture(null);
                }
                return warpManager.removeWarp(warpName).thenRun(() -> {
                    player.sendMessage(langManager.getMessageFor(player, "delwarp.success",
